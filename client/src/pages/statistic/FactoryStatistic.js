...
function FactoryStatistic() {
    ...
    if (dataFiltered) {
        if (productState !== '-1') {
            dataFiltered = dataFiltered.filter(product => (product.status == productState));
            if (dataFiltered) {
           ...
           <div className="flex mt-5">
            <div className="basis-1/4">
                <div className="w-2/3 mx-auto">
                    <label for="countries" className="block mb-2 text-base font-medium text-blue-600 dark:text-white">Trạng thái</label>
            ...
            <div className="basis-1/4">
                <div className="w-2/3 mx-auto">
                    <label for="countries" className="block mb-2 text-base font-medium text-blue-600 dark:text-white">Năm</label>
                        <select id="countries" className="bg-gray-50 border border-gray-300 text-blue-800 font-medium text-sm rounded-lg ring-1 focus:ring-blue-500
                                                    focus:border-blue-500 focus:outline-none block w-full py-3 px-1"
                                                    onChange={yearChange}>
            ...
            <div className="basis-1/4">
                <div className="w-2/3 mx-auto">
                    <label for="countries" className="block mb-2 text-base font-medium text-blue-600 dark:text-white">Quý</label>
                        <select className="bg-gray-50 border border-gray-300 text-blue-800 font-medium text-sm rounded-lg ring-1 focus:ring-blue-500
                                    focus:border-blue-500 focus:outline-none block w-full py-3 px-1"
                                    onChange={quarterChange}
                                    disabled = {year == '0' ? true : false}>
                            <option value="-1"></option>
                            { quarters && quarters.map(quart => {
                                return (<option value={quart - 1} key={quart}> Quý {quart}</option>)
                            })}
                        </select>
                </div>
            </div>
            <div className="basis-1/4">
                <div className="w-2/3 mx-auto">
                    <label for="countries" className="block mb-2 text-base font-medium text-blue-600 dark:text-white">Tháng</label>
                        <select id="countries" className="bg-gray-50 border border-gray-300 text-blue-800 font-medium text-sm rounded-lg ring-1 focus:ring-blue-500
                                                focus:border-blue-500 focus:outline-none block w-full py-3 px-1"
                            ...
                            {months && months.map (mon => {
                                return (<option value={mon} key={mon}> Tháng {mon}</option>)
                            })}
                        </select>
                </div>
            </div>
