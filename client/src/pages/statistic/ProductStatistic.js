...
function ProductStatistic() {
  ...
  if (user.role === 2) {
    ...
    stateData = stateList.filter(state => (state.id === 1 || state.id === 3 || state.id === 5 || state.id === 6 || (state.id >= 9 && state.is <= 11))).map(state => {
  ...
  if (dataFiltered) {
    if (productState !== "-1") {
      if (productState === '3') {
        dataFiltered = dataFiltered.filter(product => (product.status == 3 || product.status == 4 || product.status == 7 || product.status == 8)).map(product =>{
  ...
  var YearData;
  if (listProduct) {
    YearData = listProduct.filter((data) => {
        ...
        <div className="basis-1/4">
          <div className="w-2/3 mx-auto">
            ...
            <select
              ...
              {stateData.map(state => {
                return (<option value={state.id} key={state.id}>{state.type}</option>)
              })}
            </select>
          </div>
        </div>
